...
| フェーズ1: Runtime最小縦断 | 実施中 | Runtime API/Tag ServerのエラーJSON契約を固定化し、Runtime UIで `error` / `publish_errors` / statusフォールバック表示をE2Eで検証済み。OpenAPIは `components.responses` / `components.schemas` で成功・失敗応答を共通化済み |
