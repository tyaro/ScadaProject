...
| フェーズ1: Runtime最小縦断 | 実施中 | Runtime API/Tag ServerのエラーJSON契約を固定化し、Runtime UIで `error` / `publish_errors` / statusフォールバック表示をE2Eで検証済み。OpenAPIは `components.responses` / `components.schemas` で成功・失敗応答とrequestBody入力DTOを共通化済み。Tag Serverの操作ログ最小保存（`GET /api/v1/operation-logs`）、Preview RuntimeのSVG modify rules最小投影（`modifiers`）、Runtime UIでの `modifiers` 最小反映（色・表示・文字）、MQTT delta時のmodifier再評価、`modify_rules.condition` の比較/範囲/複合条件評価、演算子別入力バリデーション厳格化、conditionバリデーションのコード付き詳細エラー形式（`code/path/detail`）、Runtime UIでのコード解釈とユーザー向け表示マッピング、Builder/Runtime共通エラーコード一覧と表示マッピング契約の文書化、Builder APIでの共通マッピング実装（未知コードフォールバック含む）、Builder APIの構造化JSONマッピング出力（`user_message`, `known_code`）とその仕様文書化、Builder UI向けHTTPエンドポイント `POST /api/v1/errors/map` と最小 Builder UI エラー表示ラボ（既知/未知コードE2E込み）、`mock-main` 相当の最小 Screen Object Editor へのフォーカス誘導、condition本体入力（`op/value/min/max/values`）、screen-definition 最小シリアライズJSONプレビュー、Load/Download による最小 file I/O、Builder API経由の project screen 読込/保存（`GET/PUT /api/v1/screens/{screen_id}`）、Builder API経由の Save As 保存（`POST /api/v1/screens/save-as`）、Builder UI から Preview Runtime projection API を呼ぶ最小プレビュー導線（Builder-Runtime 横断）、screen保存時のJSON Schema厳密検証（`screen-definition.schema.json`）、Builder UI の project metadata 編集欄と保存先パス可視化、不正 `screen_id` / `relative_path` の即時ブロック、Builder UIの契約型 fetch（`apps/builder-ui/src/contracts/builderApi.ts`）、Builder API独立OpenAPI契約 `contracts/openapi/builder.yaml`、契約整合用の Builder API 境界テストとローカルCI入口への Builder OpenAPI 必須パス検証、標準 `check_local_ci` 経路の再通過、`RUN_SUPERVISE_LOG_CHECKS=1` による `tauri-shell` 監視ログ集計CLI回帰チェック追加、workflow_dispatch 入力 `run_supervise_log_checks` によるクラウド任意実行対応、CIのNode20警告対策（actionsメジャー更新、強制Node24オーバーライド削除、push/workflow_dispatch両経路でwarning-free実測）、夜間スケジュールによる拡張チェック定期実行（bind依存 + Runtime UI E2E + Builder UI E2E 自動化）、CIジョブ上限時間の明示（`timeout-minutes`）、workflow concurrency制御による重複実行抑止、`tauri-shell` supervise-loop の `--supervise-log-dir` による要約JSONL/サービス別ログ永続化、`--supervise-log-summary` / `--supervise-log-summary-json` による運用集計CLI、`fail-on` オプションによる監視異常時非0終了まで完了 |
...
Builder UI と Runtime API のようにフェーズ境界を跨ぐ実装は、主目的を基準にフェーズへ計上しつつ、`docs/progress.md` に「Builder-Runtime 横断」と明記して追跡する。初期段階では、保存導線、プレビュー反映、契約整合テストを横断トラックとして管理する。
