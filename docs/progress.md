...
- Svelte監視画面のブラウザ自動検証を追加
  ...
  - Builder UI の `Supervisor Log Summary` に `fail on parse error` / `fail on service exit` トグルを追加し、CLI の `fail-on` 運用に近い判定ポリシーを画面上で切り替えられるようにした
  ...
  - Supervisor policy のソース表示（`Source: url | localStorage | default`）を追加し、起動時・手動変更後の有効設定の由来を画面上で追跡できるようにした
  - `Applied: <ISO timestamp>` バッジを追加し、policy が最後に適用された時刻を source 表示と並べて確認できるようにした
  - health 表示を `flagged_issues`（トグル反映後）と `raw_issues`（生集計）に分離し、判定ルールと実測値を同時に追えるようにした
  - `apps/builder-ui` の E2E を15ケースへ拡張し、Tauri bridge経由の supervise summary 表示、policy toggle再評価、policy preset適用、policy永続化、URL query連携、policy source表示回帰を固定化した
  ...
  - `.github/workflows/ci.yml` に `tauri-shell` exhausted-restart ignored テスト専用の監視ジョブ（`Tauri Shell Exhausted-Restart Monitor`）を追加し、nightly と workflow_dispatch 入力で個別実行できるようにした
...
- `tauri-shell` 永続化監視ログの集計CLI追加（最小版）
  ...
  - `--supervise-log-summary-fail-on-parse-error` を追加し、`parse_errors > 0` で非0終了できるようにした
  - `--supervise-log-summary-fail-on-service-exit` を追加し、`exited=true` または `started=false` を検出したら非0終了できるようにした
...
1. `gh workflow run ci.yml` の結果を定期レビューし、`schedule` 実行（nightly）で Builder/Runtime 拡張チェックが連続成功しているかを追跡する。
2. `check_local_ci.sh` の `timeout` コマンド未導入環境（macOS標準）でも待機制御できるよう、`gtimeout` へのフォールバックか導入手順を整備する。
3. `workflow_dispatch` で `run_tauri_shell_exhausted_restart_test=true` を定期的に実行し、専用監視ジョブのログ蓄積と失敗傾向を確認する。
...
- `cd apps/builder-ui && npm run check`: 成功（Supervisor Summary fail policy toggle 追加後）
- `cd apps/builder-ui && npm run test:e2e`: 成功（11 passed、policy toggle による `Issue detected -> Healthy` 再評価回帰を含む）
- `cd apps/builder-ui && npm run check`: 成功（Supervisor Summary policy preset 追加後）
- `cd apps/builder-ui && npm run test:e2e`: 成功（12 passed、`strict/balanced/observe` preset適用回帰を含む）
- `cd apps/builder-ui && npm run check`: 成功（Supervisor policy localStorage永続化追加後）
...
- `cd apps/builder-ui && npm run check`: 成功（policy source 表示追加後）
...
- `ruby -e "require 'yaml'; YAML.load_file('.github/workflows/ci.yml')"`: 成功（`Tauri Shell Exhausted-Restart Monitor` ジョブ追加後）
- `cargo test -p tauri-shell --test supervise_loop_reset supervise_loop_fails_when_restart_is_exhausted_with_fail_flag -- --ignored`: 成功（専用監視ジョブと同一コマンドで 1 passed）
